...
argv <- "btpd_pres_abs_pts_usgs_species_viewer"
...
# CLIMATE VARIABLES
bioclim                   <- raster::stack(list.files(paste(HOME,"/Products/weather/worldclim/30_sec/bioclim/",sep=""),full.names=T,pattern="bio_13[.]bil$|bio_4[.]bil$"))
...
# Calculate and aggregate our landscape metrics and other explanatory variables into a data.frame
 ...
 t_landcover_grass_presences <- cbind(resp=1,calculateLMetricsForFocalCover(get("grass_presences",session_data), metrics=c("total.area")));
	t_landcover_grass_absences  <- cbind(resp=0,calculateLMetricsForFocalCover(get("grass_absences",session_data), metrics=c("total.area")));
