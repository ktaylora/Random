...
argv <- "swfox_pres_abs_pts_usgs_species_viewer"
...
spp <- readOGR("/home/ktaylora/PLJV/species_data/usgs_gap_species_ranges/vector/",argv[1], verbose=F)
...
if(length(ls(pattern="absences_topography")) == 0){
    ...
    rm(t_27);
    assign("absences_topography", value=absences_topography,envir=session_data)
    assign("presences_topography", value=presences_topography,envir=session_data)
    save(list=ls(session_data), envir=session_data, file=s, compress=T);  
}; 
# Calculate human dimensions variables
if(sum(ls(session_data)=="distRoads_absences")==0){  
  distance_to_roads         <- raster("/home/ktaylora/PLJV/distanceToRoads.epsg4326.resampled.cropped.tif")
...
# Calculate and aggregate our landscape metrics and other explanatory variables into a data.frame
 ...
 if(sum(ls(session_data)=="t")==0){
	...
	t_landcover_shrub_absences  <- cbind(resp=0,calculateLMetricsForFocalCover(get("shrub_absences",session_data), metrics=c("total.area")));
	...
	t_1 <- cbind(t_landcover_shrub_presences[,1:3], t_landcover_ag_presences[,3])
                    names(t_1) <- c("resp","id","total.area_shrub","total.area_ag")
        t_2 <- cbind(t_landcover_shrub_absences[,1:3], t_landcover_ag_absences[,3])
                    names(t_2) <- c("resp","id","total.area_shrub","total.area_ag")
	...
	t <-cbind(t,rbind(get('presences_topography', envir=session_data), get('absences_topography', envir=session_data)))
        ...
        t <- cbind(t,rbind(get('distRoads_presences',envir=session_data),get('distRoads_absences',envir=session_data)))
