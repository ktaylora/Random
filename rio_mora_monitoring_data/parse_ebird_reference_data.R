...
if(!dir.exists("~erd")){
  dir.create("~/erd")
  system("tar xzf ~/Incoming/ebird_number_crunching/count_data/erd_us48_data_grouped_by_year_v5.0.tar.gz ~/erd");
}
...
for(i in 2002:2012){
  ...
  t <- sqldf("SELECT * from f WHERE LONGITUDE <= -100.6 and LONGITUDE >= -108.735
              ...
              and COUNT_TYPE not in ('P22','P34')", # P22 and P34 are eBird's "Traveling" designators
             dbname = tempfile(),
...
s <- SpatialPointsDataFrame(data.frame(x=merged$LONGITUDE, y=merged$LATITUDE),data=data.frame(year=merged$YEAR,day=merged$DAY))
  writeOGR(s,paste(Sys.getenv("HOME"),"/Incoming/ebird_number_crunching",sep=""),"pts_2002_2012",driver="ESRI Shapefile", overwrite=T)
  write.csv(merged,paste(Sys.getenv("HOME"),"/Incoming/ebird_number_crunching/pts_2002_2012.csv",sep=""))
